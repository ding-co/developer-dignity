...
- html 태그; 루트 노드
  ...
  - head 태그
    - meta charset UTF-8; 여러 언어 깨질 수 있음
  - body 태그
...
- a 태그
  ...
  - a태그에 href 속성으로 tel:+821000000 세팅하면 <br/>
...
- input 태그
  ...
  - type="search"
    - 자동으로 x 표시 나옴
    - x 누르면 입력값 사라짐
    - ex. 검색하다가 지울 때 불편함 해소 (입력값 자주 초기화할 때 사용)
  - type="tel"
    - 전화번호 입력
...
- SPA; FE framework (바뀌는 부분만 적용 가능)
  ...
  - 과거 iframe 태그; 화면 깜빡임 방지 용도 등으로 썼음
    - 만약 iframe 없으면 상단 메뉴에 중복된 코드 다 작성해야 했음 <br/>
      (만약 페이지로 이동하는 Hard Coding 했을 때 힘들었음..)
